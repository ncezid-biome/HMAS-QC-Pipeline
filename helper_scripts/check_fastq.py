...
def main():
    ...
    parser.add_argument('-r1', '--read1', metavar = '', required = True, \
                        help = 'Specify R1 fastq file input to QC Pipeline (IDs must start with @M)') 
    parser.add_argument('-r2', '--read2', metavar = '', required = True, \
                        help = 'Specify R2 fastq file input to QC Pipeline (IDs must start with @M)')
    parser.add_argument('-a', '--after', metavar = '', required = True, \
                        help = 'Specify fasta file output from QC Pipeline')
    ...
    # Then we need to get the sequence info for these IDs (R1, R2 sequences)
    ...
    for i in random_sample:
        ...
        d[key] = seq.strip() # Key =  read ID, value = sequence
        
    # For the read IDs in dict, get the R1 & R2 sequences & append them to the dict value list
    with open(raw1, 'r') as f:
        for line in f:
            if line.startswith('@M'):
                id = re.sub(':', '_', line)
                if id.strip().split()[0][1:] in d.keys():
                    append_value(d, id.strip().split()[0][1:], next(f).strip())
    ...
    # Turn reads dictionary into a dataframe
    reads = pd.DataFrame([(k, *v) for k, v in d.items()])
    reads.columns = ('id', 'seq_after_QC', 'R1', 'R2','primer_name')
 
    # Turn primers dictionary into a dataframe
    primers = pd.DataFrame([(k, *v) for k, v in o.items()])
    primers.columns=('primer_name', 'fwd', 'fwd_rc', 'rev', 'rev_rc')

    # Merge the primer sequence info into the sample of read IDs
    full = pd.merge(reads, primers, on = 'primer_name', how = 'left')
    
    # Get the R1, R2 fragments with 5' and 3' (read-through) primers trimmed off
    full = full.assign(fwd_len = full.fwd.str.len())
    full = full.assign(rev_len = full.rev.str.len())
    full['R1_frag'] = full.apply(lambda x: x.R1[x.fwd_len:],axis=1)
    full['R2_frag'] = full.apply(lambda x: x.R2[x.rev_len:],axis=1)
    ...
    new_full= full[['id', 'seq_after_QC', 'R1', 'fwd', 'rev_rc', 'R2', 'segment', 'primer_name', 'fwd_rc', 'rev']]
