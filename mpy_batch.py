...
def main(config):
    ...
    # Note: probably should use new_group here too, and check for the empty accnos file after this step just in case
    # Also need to clean up the log file, and clean up millions of intermediate .map files generated by this precluster step
    m.pre.cluster(fasta='current', name='current', group='current')
    ...
    # Can't use group and count together; need to do it in two lines
    m.get.seqs(fasta='current', accnos='current', count='current')
    m.get.seqs(fasta='current', accnos='current', group='current')
    #m.get.seqs(fasta='current', accnos='current', count='current', group='current')

    m.summary.seqs(fasta='current', count='current')

    m.rename.file(fasta='current', group='current', count='current', list='current', prefix=config.get('rename_param', 'prefix') +'.final')
